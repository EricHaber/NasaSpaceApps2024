...
def visualize_planet_positions_and_orbits(positions, planet_colors):
    plotter = pv.Plotter()
    ...
    planet_sizes = {
        'Mercury': 0.03,  # Small inner planets
        'Venus': 0.075,
        'Earth': 0.08,
        'Mars': 0.04125,
        'Jupiter': 0.8375,  # Larger outer planets
        'Saturn': 0.7,
        'Uranus': 0.675,
        'Neptune': 0.675,
        'Ceres': 0.015,
    ...
    for i in range(len(planets)):
        if type(planet_colors[planets[i]]) == tuple:
...
with col1:
    if st.button('Sun', use_container_width=True):
        planet_colors ['Sun']= (0, 255, 0)
with col2:
    if st.button('Mercury', use_container_width=True):
        planet_colors['Mercury'] = (0, 255, 0)

with col3:
    if st.button('Venus', use_container_width=True):
        planet_colors['Venus'] = (0, 255, 0)
with col4:
    if st.button('Earth', use_container_width=True):
        planet_colors['Earth'] = (0, 255, 0)
with col5:
    if st.button('Mars', use_container_width=True):
        planet_colors['Mars'] = (0, 255, 0)
with col1:
    if st.button('Jupiter', use_container_width=True):
        planet_colors['Jupiter'] = (0, 255, 0)
with col2:
    if st.button('Saturn', use_container_width=True):
        planet_colors['Saturn'] = (0, 255, 0)
with col3:
    if st.button('Uranus', use_container_width=True):
        planet_colors['Uranus'] = (0, 255, 0)
with col4:
    if st.button('Neptune', use_container_width=True):
        planet_colors['Neptune'] = (0, 255, 0)
with col5:
    if st.button('Halley\'s comet', use_container_width=True):
        planet_colors['Halley\'s comet'] = (0, 255, 0)
with col1:
    if st.button('Ceres', use_container_width=True):
        planet_colors['Ceres'] = (0, 255, 0)
with col2:
    if st.button('Pallas', use_container_width=True):
        planet_colors['Pallas'] = (0, 255, 0)
with col3:
    if st.button('Vesta', use_container_width=True):
        planet_colors['Vesta'] = (0, 255, 0)
with col4:
    if st.button('Hygiea', use_container_width=True):
        planet_colors['Hygiea'] = (0, 255, 0)
with col5:
    if st.button('Eros', use_container_width=True):
        planet_colors['Eros'] = (0, 255, 0)
with col1:
    if st.button('Itokawa', use_container_width=True):
        planet_colors['Itokawa'] = (0, 255, 0)
with col2:
    if st.button('Bennu', use_container_width=True):
        planet_colors['Bennu'] = (0, 255, 0)
with col3:
    if st.button('2002 AA69', use_container_width=True):
        planet_colors['2002 AA69'] = (0, 255, 0)
with col4:
    if st.button('2004 BL86', use_container_width=True):
        planet_colors['2004 BL86'] = (0, 255, 0)
with col5:
    if st.button('2014 UR116', use_container_width=True):
        planet_colors['2014 UR116'] = (0, 255, 0)


if "leaveToObjectPage" not in st.session_state:
    st.session_state.leaveToObjectPage = False
if "wereto" not in st.session_state:
    st.session_state.wereto = ""
if (0, 255, 0) in planet_colors.values():
    for planet, color in planet_colors.items():
        if planet_colors[planet] == (0, 255, 0):
            if planet == "Halley\'s comet":
                st.session_state.wereto = f"pages/Halley.py"
                #st.session_state.leaveToObjectPage = st.button("Go To Halley\'s comet page", use_container_width=True)
            elif planet == "2004 BL86":
                st.session_state.wereto = f"pages/2004.py"
            elif planet == "2002 AA69":
                st.session_state.wereto = f"pages/2002.py"
            elif planet == "2014 UR116":
                st.session_state.wereto = f"pages/2014.py"
            else:
                st.session_state.wereto = f"pages/{planet}.py"
                #st.session_state.leaveToObjectPage = st.button(f"Go To {planet} page", use_container_width=True)

if st.session_state.wereto != "":
    if "Halley" in st.session_state.wereto:
        st.session_state.leaveToObjectPage = st.button("Go To Halley\'s comet page", use_container_width=True)
    else:
        st.session_state.leaveToObjectPage = st.button(f"Go To {st.session_state.wereto.removeprefix("pages/").removesuffix(".py")} page", use_container_width=True)
    #st.session_state.leaveToObjectPage = st.button("Go To Halley\'s comet page", use_container_width=True)

if st.session_state.leaveToObjectPage:
    st.write(st.session_state.wereto)
    st.switch_page(st.session_state.wereto)

positions = compute_planet_positions(d)
visualize_planet_positions_and_orbits(positions,planet_colors)

st.write("""
The planets are not to scale but are relative to each other are correct other except for the sun, comets and astroids.""")
